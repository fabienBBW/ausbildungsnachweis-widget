...
class Day {
    ...
    public static function fromRaw(int $day_id, string $day_date, string $day_activities_json) {
        $day = new static();
        $day->day_id = $day_id;
    ...
    public function save() {
        ...
        if($this->fromId($day_id) == false) {
            ...
            $stmt = $pdo->prepare("INSERT INTO days (day_id, day_date, day_activities_json) VALUES(?, ?, ?)");
            $ret_val = $stmt->execute([$this->day_id, $this->day_date, $this->day_activities_json]);
            if($ret_val == false) {
                return false;
            }
            return true;
        } else {
            $stmt = $pdo->prepare("UPDATE days SET day_date = ?, day_activities_json = ? WHERE day_id = ?");
            $ret_val = $stmt->execute([$this->day_date, $this->day_activities_json, $this->day_id]);
            if($ret_val == false) {
                return false;
            }
            return true;
